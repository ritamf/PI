...
def register_user_page(request):
    ...
    try:
        ...
        # h.hexdigest()
        sObj = Users.if_not_exists().create(user_name_value=user_name, user_email_value=user_email,
                                            user_password_value=h.hexdigest())
    except:
...
def insert_into_db(request, user_token, sensorid):
    ...
    if sessCache is None:
        ...
        user_session = DB.sessionLogin(request.session.get('username'), Users.objects.get(
            user_email_value=request.session.get('usermail')).user_password_value)
        cache.add(user_session[0], user_session[1])
        sessCache = cache.get(request.session.get('username'))
...
def datasource_query(request):
    user = cache.get(request.session.get('username'))
    ...
    for target in data['targets']:
        ...
        if campo is None or campo == '':
            sensorAttributes = DB.getSensorAttributes(user, sensor)
        ...
        if req_type == 'table':
            results.append(dataframe_to_json_table(target, values, intervalMs, ti, tf))
        else:
            results.append(dataframe_to_response(target, values, intervalMs, ti, tf))
...
def dataframe_to_response(target, results, freq, ti, tf):
    ...
    campos = results[0].copy(avoid='timestamp')
